...
contract SmartBacker {
    ...
    function createProject(
        ...
        //require(expiresAt > block.timestamp, "Expiration date must be in the future");
    ...
    function backProject(uint id) public payable returns (bool) {
        ...
        if(projects[id].status != statusEnum.DELETED && block.timestamp >= projects[id].expiresAt) {
